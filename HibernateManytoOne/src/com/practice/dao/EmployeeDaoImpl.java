...
public class EmployeeDaoImpl implements EmployeeDao {
	...
	public void deleteEmployee(int empId) {
		...
		try{
			...
			if(emp!=null){
				session.getTransaction().begin();
				emp.setAccount(null);//right way to delete the employee as first we need to set the value of the referenced column to null
				session.delete(emp);
				session.getTransaction().commit();
			}
	...
	public void deleteAccount(int accountNumber) {
		...
		Session session=sf.openSession();
		try{
			AccountEntity acc=(AccountEntity)session.get(AccountEntity.class,accountNumber);
			if(acc!=null){
				
				/*for(EmployeeEntity emp:acc.getList()){
					emp.setAccount(null);
				}*/
				acc.setList(null);
				session.getTransaction().begin();
